...
System.Clear()
System.Title("ATOMIC - By LEO KODS")
...
async def webhookTools():
    ...
    webhook_txt = '''
        ...
        2. Spam com webhooks +1 (usando webhooks.txt)
        3. Excluir webhook
    ...
    if ch == '1':
        print_logo()
        print_('[=] Insira o URL do webhook para spam')
        url = input_()
        print_('[=] Digite mensagem para spam')
        message = input_()
        print_('[=] Insira a contagem de spam')
        count = input_()
        print_('[??] você está disposto a usar proxy? [S,N]')
        proxy = input_()
        ...
        if not proxy or not proxy.lower().startswith('y'):
            ...
            Write.Print('\n     [??] Quantos proxies você deseja usar? ', Colors.yellow_to_green, interval=0.01)
            amount = input_()
                            ...
                            for i in p_:
                                if len(checked) < int(amount):    
                                    try:
                                        ...
                                        print_('Adicionado 1 à lista verificada')
                                        checked.append(i)
                                    except: 
                                        print_err('Falha ao conectar com este')
                            else:
                                print_logo()
                                if len(checked) > 0:
                                    for i in range(int(count)):
    ...
    if ch == '2':
        ...
        if len(w) > 0: 
            ...
            if not msg or not count or not count.isalnum() or not len(w) >0: await webhookTools()
                ...
                if not proxy_usage.lower().startswith('y'):
                    ...
                    print_('[??] Quantos proxies você deseja usar?')
                    amount = input_()
                    with open('proxies.txt', 'r') as file:
                        p = file.readlines()
                        if len(p) < 0:
                            ...
                            for i in p_:
                                if len(checked) < int(amount):    
                                    try:
                                        ...
                                        ip = req.get("https://api.myip.com", proxies=prs)
                                        print_success(f'Adicionou 1 à lista verificada{str(i)}')
                                        checked.append(i)
                                    except: 
                                        print_err(f'Falha ao conectar com este') 
                            else: 
                                checked_hooks = []
                                print()
                                print_logo()
                                print_('[!!] Verificando Webhooks')
                                for i in webhooks: 
                                    if check(i):
                                        print_success(i)
                                        checked_hooks.append(i)
                                    ...
                                    if len(checked) > 0:
                                        print_logo()
                                        print_(f'comecei a enviar spam com f{len(checked_hooks)} webhook(s)')
                                        for i in range(int(count)):
        ...
        print_logo()
        print_('[=] Digite o webhook que você deseja excluir')
        web = input_()
        print_logo()
        try:
            req.delete(web)
            print_success(f'Excluído {web}')
        except: 
            print_err(f'Falha ao excluir {web}')
        finally:
            time.sleep(3)
            await webhookTools()
    elif ch == '4':
        ...
        if len(w) > 0:
            print_logo()
            print_(f'[??] tem certeza de que deseja excluir webhook(s) {len(w)}? [S,N]')
            ch_ = input_()
            if ch_.lower().startswith('y'):
                print_logo()
                print_(f'[!!] Começou a excluir {len(w)} webhook(s)')
                for i in w:
                    try:
                        req.delete(i)
                        print_success(f'Deletado{i}')
                    except: 
                        print_err(f'Falha ao excluir {i}')
                else:
                    print_('[??] Deseja remover o arquivo “webhooks.txt”? [S,N]')
                    ch__ = input_()
